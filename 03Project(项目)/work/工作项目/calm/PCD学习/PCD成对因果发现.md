---
url: https://gemini.google.com/app/7db0d5ed6effaf3c
学习辅导: https://gemini.google.com/gem/learning-coach/8643268cd00c17af
维基百科: https://zh.wikipedia.org/wiki/%E5%9B%A0%E6%9E%9C%E6%8E%A8%E6%96%B7
---
只要事件 A 的发生通过某种机制改变了事件 B 发生的概率（且这种改变不是由混杂因子引起的），我们就认为 A 是 B 的原因之一。
...
- **统计[[独立性]]（Statistical Independence）：** 必须深刻理解 $P(X, Y) = P(X)P(Y)$ 的定义以及如何在连续变量中度量它。
    
- **高阶[[矩]]（Higher-order Moments）：** 均值和[[方差]]不足以描述非高斯分布。你需要掌握偏度（Skewness）和峰度（Kurtosis），因为许多 PCD 方法（如 LiNGAM）依赖于数据的非高斯性（Non-Gaussianity）。
...
> [!important]
> 如果正向模型 $Y = f(X) + N$ 成立且 $f$ 是非线性的，那么反向模型 $X = g(Y) + N'$ **一定**得不到独立的噪声。
